...
def _get_notebook_params(**kwargs):
    ...
    """
    ti = kwargs["ti"]
    file_path = os.path.join(_BASE_PATH, "configs", f"daily_ministock_{kwargs['env']}.json")
...
with DAG(f"{os.path.basename(__file__).replace('.py', '')}_api",
    ...
    tags=["auto_report", "ministock", "all", "daily"]
    ) as dag_api:
    ...
    with TaskGroup(group_id="main_tasks") as main_tasks:
        ...
        ministock_gad_api = DatabricksRunNowOperator(
            task_id="ministock_gad_api",
        ...
        ministock_fb_api = DatabricksRunNowOperator(
            task_id="ministock_fb_api",
        ...
        ministock_tik_api = DatabricksRunNowOperator(
            task_id="ministock_tik_api",
        ...
        main_tasks_start >> [ministock_gad_api, ministock_fb_api, ministock_tik_api]
...
with DAG(f"{os.path.basename(__file__).replace('.py', '')}_sql",
    ...
    with TaskGroup(group_id="main_tasks") as main_tasks:
        ...
        ministock_postback_sql = DatabricksRunNowOperator(
            task_id="ministock_postback_sql",
            job_id="{{ ti.xcom_pull(task_ids='get_notebook_params', key='postback_job_id') }}",
            notebook_params={
                "env": env
            },
            trigger_rule=TriggerRule.ALL_SUCCESS
        ...
        main_tasks_start >> ministock_postback_sql
