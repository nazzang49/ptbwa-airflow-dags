...
def _get_notebook_params(**kwargs):
    ...
    """
    ti = kwargs["ti"]
    file_path = os.path.join(_BASE_PATH, "configs", f"daily_welrix_{kwargs['env']}.json")
...
def _get_validation_config(**kwargs):
    ...
    table_names = [
        "nsa_welrix_adkeyword_daily_raw",
...
with DAG(os.path.basename(__file__).replace(".py", ""),
    ...
    tags=["auto_report", "welrix", "all", "daily"]
    ) as dag:
    ...
    with TaskGroup(group_id="main_tasks") as main_tasks:
        welrix_bq_api = DatabricksRunNowOperator(
            task_id="welrix_bq_api",
            job_id="{{ ti.xcom_pull(task_ids='get_notebook_params', key='bq_job_id') }}",
            notebook_params={
                "env": env
            },
            trigger_rule=TriggerRule.ALL_SUCCESS
        ...
        welrix_ga_traffic_stat_sql = DatabricksRunNowOperator(
            task_id="welrix_ga_traffic_stat_sql",
            job_id="{{ ti.xcom_pull(task_ids='get_notebook_params', key='ga_traffic_stat_job_id') }}",
            notebook_params={
                "env": env
            },
            trigger_rule=TriggerRule.ALL_SUCCESS
        ...
        welrix_ga_groupby_stat_sql = DatabricksRunNowOperator(
            task_id="welrix_ga_groupby_stat_sql",
            job_id="{{ ti.xcom_pull(task_ids='get_notebook_params', key='ga_groupby_stat_job_id') }}",
            notebook_params={
                "env": env
            },
            trigger_rule=TriggerRule.ALL_SUCCESS
        ...
        welrix_nsa_api = DatabricksRunNowOperator(
            task_id="welrix_nsa_api",
        ...
        welrix_ga_propfit_stat_sql = DatabricksRunNowOperator(
            task_id="welrix_ga_propfit_stat_sql",
            job_id="{{ ti.xcom_pull(task_ids='get_notebook_params', key='ga_propfit_stat_job_id') }}",
            notebook_params={
                "env": env
            },
            trigger_rule=TriggerRule.ALL_SUCCESS
        ...
        [welrix_bq_api, welrix_nsa_api] >> welrix_ga_traffic_stat_sql >> welrix_ga_groupby_stat_sql >> welrix_ga_propfit_stat_sql
