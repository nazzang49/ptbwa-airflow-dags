...
with DAG(f"{os.path.basename(__file__).replace('.py', '')}_api",
    ...
    with TaskGroup(group_id="main_tasks") as main_tasks:
        ...
        # @jinyoung.park
        kcar_criteo_api = DatabricksRunNowOperator(
            task_id="kcar_criteo_api",
            job_id="{{ ti.xcom_pull(task_ids='get_notebook_params', key='criteo_job_id') }}",
            notebook_params={
                "env": env,
                "data_interval_end": "{{ ti.xcom_pull(task_ids='check_data_interval', key='data_interval_end') }}"
            },
        ...
        main_tasks_start >> [kcar_gad_api, kcar_kmt_api, kcar_free_api, kcar_gsh_api, kcar_nsa_api, kcar_criteo_api]
...
with DAG(f"{os.path.basename(__file__).replace('.py', '')}_sql",
    ...
    with TaskGroup(group_id="main_tasks") as main_tasks:
        ...
        main_tasks_start >> kcar_report_stat_sql
