...
def _check_env_before_main_proc(**kwargs):
    """
...
def _set_additional_validation_config(**kwargs):
    """
...
def _check_env_after_main_proc(**kwargs):
    """
    A method for checking env to branch
    """
    if kwargs["env"] == "dev":
...
with DAG('autoreport_kcar_dag',
    ...
    check_env_before_main_proc = BranchPythonOperator(
        task_id="check_env_before_main_proc",
        python_callable=_check_env_before_main_proc,
        op_kwargs={"env": env},
        trigger_rule=TriggerRule.ALL_SUCCESS
    )
    ...
    set_additional_validation_config = PythonOperator(
        task_id="set_additional_validation_config",
        python_callable=_set_additional_validation_config,
        op_kwargs={"env": env},
        trigger_rule=TriggerRule.ALL_SUCCESS
    )
    ...
    check_validation = AutoReportValidationOperator(
        ...
        notebook_task="{{ ti.xcom_pull(task_ids='get_validation_config', key='validation_config') }}",
        databricks_conn_id='databricks_default',
        existing_cluster_id="1026-083605-h88ik7f2",
        trigger_rule=TriggerRule.ALL_SUCCESS,
        total_period="{{ ti.xcom_pull(task_ids='get_total_period', key='total_period') }}",
        table_names="{{ ti.xcom_pull(task_ids='set_additional_validation_config', key='table_names') }}",
        project="AUTOREPORT" if env == "prod" else "AUTOREPORT_TEST"
    )

    check_env_after_main_proc = BranchPythonOperator(
        task_id="check_env_after_main_proc",
        python_callable=_check_env_after_main_proc,
        op_kwargs={"env": env},
        trigger_rule=TriggerRule.ALL_SUCCESS
    )
    ...
    # dev
    update_jobs_from_prod_to_dev >> get_total_period
    update_jobs_from_dev_to_prod >> end

    # common
    start >> get_notebook_params >> get_validation_config >> check_env_before_main_proc >> [update_jobs_from_prod_to_dev, get_total_period]
    get_total_period >> databricks_run_job_group >> set_additional_validation_config >> check_validation >> check_env_after_main_proc
    check_env_after_main_proc >> [update_jobs_from_dev_to_prod, end]